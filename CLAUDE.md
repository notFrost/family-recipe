...
Lean by design: lean on Claude's built-ins instead of reimplementing them. Use the `Plan` agent / plan mode for architecture, the `Explore` agent for codebase recon, `/code-review` for review, and `/verify` for verification. Backend and frontend coding happen on the main thread. The only custom subagent kept is `designer` — it adds a real constraint (mock-data-only design exploration) that the built-ins don't.
...
- Use the `frontend-design` skill for aesthetic direction, and the `designer` agent when you want this work isolated/parallelized.
- Don't run `npm run dev` (blocks the shell). To see it: screenshot the dev preview `https://family-recipe-git-dev-nfs-projects-adbffaf9.vercel.app/preview/discover` (latest pushed `dev`), or start a local preview via the Claude Preview MCP for unpushed work.
